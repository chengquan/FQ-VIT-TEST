...
class BaseQuantizer(nn.Module):
    ...
    def getScaleandZeroPoint(self, inputs, scale=None, zero_point=None):
        raise NotImplementedError

    def forward(self, inputs):
        quant_o = self.quant(inputs)
        outputs = self.dequantize(quant_o)
        scale, zero_point = self.getScaleandZeroPoint()
        return scale, zero_point, quant_o, outputs
