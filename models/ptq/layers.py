...
class QConv2d(nn.Conv2d):
    ...
    def __init__(self,
        ...
        self.enquantdata = torch.empty(1).to('cpu')
        self.dequantdata = torch.empty(1).to('cpu')
        self.scale = torch.empty(1).to('cpu')
        self.zero_point = torch.empty(1).to('cpu')
        self.dataout = torch.empty(1).to('cpu')
        self.bias_data = self.bias.detach().numpy()
    def forward(self, x):
        if self.calibrate:
        ...
        scale, zero_point, quant, weight = self.quantizer(self.weight)
        self.enquantdata = weight.cpu().numpy()
        self.dequantdata = quant.cpu().numpy()
        self.scale = scale.cpu().numpy()
        self.zero_point = zero_point.cpu().numpy()
        data = F.conv2d(x, weight, self.bias, self.stride, self.padding,
                        self.dilation, self.groups)
        self.dataout = data[0].cpu().numpy()
        return data
...
class QLinear(nn.Linear):
    ...
    def forward(self, x):
        ...
        self.scale = scale.cpu().numpy()
        self.zero_point = zero_point.cpu().numpy()
        data = F.linear(x, weight, self.bias)
        self.dataout = data[0].cpu().numpy()
        return data
...
class QAct(nn.Module):
    ...
    def __init__(self,
        ...
        self.scale = torch.empty(1).to('cpu')
        self.zero_point = torch.empty(1).to('cpu')       
        self.bias_data = torch.empty(1).to('cpu')  
        self.dataout = torch.empty(1).to('cpu')
    def forward(self, x):
        if self.calibrate:
        ...
        if not self.quant:
            return x
        scale, zero_point, quant, x = self.quantizer(x)
        self.enquantdata = x[0].cpu().numpy()
        self.dequantdata = quant[0].cpu().numpy()
        self.scale = scale.cpu().numpy()
        self.zero_point = zero_point.cpu().numpy()
        return x
...
class QIntLayerNorm(nn.LayerNorm):
    ...
    def __init__(self, normalized_shape, eps=1e-5, elementwise_affine=True):
        ...
        self.mode = 'ln'
        self.scalein = torch.empty(1).to('cpu')
        self.scaleout = torch.empty(1).to('cpu')
        self.weightdata = torch.empty(1).to('cpu')
        self.biasdata = torch.empty(1).to('cpu')
    ...
    def forward(self,
        ...
        if self.mode == 'ln':
            ...
            self.scalein = in_scale.cpu().numpy()
            self.scaleout = out_scale.cpu().numpy()
            ...
            B = ((self.bias.reshape(1, 1, -1) -
                  ...
                  self.weight.reshape(1, 1, -1)) / out_scale *
                 torch.pow(2, N)).round()
            self.weightdata = self.weight.cpu().numpy()
            self.biasdata = self.bias.cpu().numpy()
